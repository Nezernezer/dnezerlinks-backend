...
// 💳 ELECTRICITY BILL PAYMENT ENDPOINT
    ...
    try {
        ...
        try {
            ...
            if (vtuRes.data && (vtuRes.data.status === 'success' || vtuRes.data.Status === 'successful')) {
                ...
                return res.status(400).json({
                    success: false,
                    error: vtuRes.data?.api_response || vtuRes.data?.message || 'Provider rejected processing request'
                });
            }
        } catch (apiErr) {
            // Reverse balance on API error
            await userRef.child('balance').transaction(currentBal => (currentBal || 0) + amount);
